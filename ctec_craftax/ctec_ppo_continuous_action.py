...
import json
from args import ctec_ppo_args
...
def make_train(config):
    ...
    def train(rng):
        ...
        # import pdb;pdb.set_trace()
        # import pdb;pdb.set_trace()
        crl_params = contrastive_network.init(_rng, dummy_obs, dummy_action, dummy_future_obs, jnp.zeros((1, config["NUM_ENVS"])),  None)
        tx = optax.chain(
                optax.clip_by_global_norm(config["MAX_GRAD_NORM"]),
        ...
        def _update_step(runner_state, unused):
            ...
            def _calculate_gae(traj_batch, future_obs, last_val, last_done, init_hstate):
                ...
                def get_crl_repr(carry, transition_batch):
                    ...
                    if config["USE_SINGLE_SAMPLE"]:
                        obs_action_rep, future_obs_rep, log_temp, init_hidden = contrastive_network.apply(crl_state["crl_model"].params, obs_inpt, action_inpt, future_obs_inpt, done_inpt, init_hidden[0])
                ...
                def _get_advantages(gae_and_next_value, transition_batch):
                    ...
                    transition, future_obs, obs_action_rep, future_obs_rep, crl_rwd_mc = transition_batch
                    # import pdb;pdb.set_trace()
                ...
                adv_info, advantages = jax.lax.scan(
                    ...
                    (traj_batch, future_obs, obs_action_rep, dicounted_future_reprs, crl_rwd_mc),
                    reverse=True,
                    unroll=16,
                )
                # import pdb;pdb.set_trace()
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _td_crl_loss(model_params,
                            ...
                            all_outputs = contrastive_network.apply(
                                model_params,
                                obs_all,
                                act_all,
                                next_obs_all,
                    ...
                    def _crl_loss(model_params, traj_batch, future_obs, init_hstate):
                        ...
                        obs_action_rep, future_obs_rep, log_temp, init_hstate = contrastive_network.apply(model_params, traj_batch.obs, action_onehot, future_obs, traj_batch.done, init_hstate[0])
            ...
            if config["DEBUG"] and config["USE_WANDB"]:
                ...
                def callback(metric, update_step, traj_batch):
                    ...
                    # print(observations[:, :config["GOAL_DIM"]])
...
if __name__ == "__main__":
    args, reset_args = ctec_ppo_args(sys)
    if args.jit:
        run_ppo(args)
    else:

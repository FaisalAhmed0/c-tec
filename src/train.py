...
from src.utils.env_utils import DiscretizedDensity
...
def train(config):
    ...
    policy_kwargs = dict(
        ...
        ctec_loss_fn=config.ctec_loss_fn,
        ctec_energy_fn=config.ctec_energy_fn,
        ctec_logsumexp_coef=config.ctec_logsumexp_coef,
        normalize_repr=config.normalize_repr,
        auto_temp=config.auto_temp,
        use_etd_rwd=config.use_etd_rwd, 
        use_ctec_rwd=config.use_ctec_rwd
    )
    goal_dim = 2 if "ant" in config.game_name else 3 
    dd = DiscretizedDensity(goal_dim=goal_dim)
    print("crating ppo model and trainer")
    # pdb.set_trace()
    model = PPOTrainer(
        policy=PPOModel,
        env=venv,
...
@click.option('--rnd_use_policy_emb', default=0, type=int,
...
@click.option('--use_ctec_rwd', default=0, type=int, help='whether to use etd or ctec reward for TDD')

@click.option('--ctec_loss_fn', default="infonce", type=str, help='loss function for TDD')
@click.option('--ctec_energy_fn', default='l1', type=str, help='energy function for TDD')
@click.option('--ctec_logsumexp_coef', default=0.0, type=float, help='logsumexp penalty for TDD')
@click.option('--normalize_repr', default=1, type=int, help='whether to use normalize ther representations from the contrastive model')
@click.option('--auto_temp', default=1, type=int, help='whether to tune the temparture automatically')
@click.option('--use_etd_rwd', default=0, type=int, help='whether to use etd or ctec reward, use ctec reward by default')
# Network params
...
def main(
    ...
    adv_momentum, ext_rew_coef, int_rew_coef, int_rew_decay, int_rew_source, int_rew_norm, int_rew_momentum, int_rew_eps, int_rew_clip,
    dsc_obs_queue_len, icm_forward_loss_coef, ngu_knn_k, ngu_use_rnd, ngu_dst_momentum, rnd_use_policy_emb,
    rnd_err_norm, rnd_err_momentum, tdd_aggregate_fn, tdd_energy_fn, tdd_loss_fn, tdd_logsumexp_coef, ctec_loss_fn, ctec_energy_fn
,ctec_logsumexp_coef, normalize_repr, auto_temp, offpolicy_data, use_model_rnn, latents_dim, model_latents_dim, policy_cnn_type, policy_mlp_layers,
    ...
    rec_interval, video_length, log_dsc_verbose, env_render, use_status_predictor, use_etd_rwd, use_ctec_rwd
):

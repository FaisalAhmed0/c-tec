...
from src.algo.intrinsic_rewards.tdd import TDDModel
from src.algo.intrinsic_rewards.ctec import CTeCModel
...
from src.algo.intrinsic_rewards.e3b import E3BModel
...
class PPOModel(ActorCriticCnnPolicy):
    def __init__(
        ...
        self.tdd_aggregate_fn = tdd_aggregate_fn
        self.tdd_energy_fn = tdd_energy_fn
        self.tdd_loss_fn = tdd_loss_fn
        self.tdd_logsumexp_coef = tdd_logsumexp_coef
        self.offpolicy_data = offpolicy_data
        self.ctec_energy_fn = ctec_energy_fn
        self.ctec_loss_fn = ctec_loss_fn
        self.ctec_logsumexp_coef = ctec_logsumexp_coef
        self.normalize_repr = normalize_repr
        self.auto_temp = auto_temp
        self.use_etd_rwd = use_etd_rwd
        self.use_ctec_rwd = use_ctec_rwd
        if isinstance(observation_space, gym.spaces.Dict):
            if "rgb" in observation_space.spaces:
                observation_space = observation_space["rgb"]
            elif "observation" in observation_space.spaces:
        ...
        if self.int_rew_source == ModelType.TDD:
            self.int_rew_model = TDDModel(
                **int_rew_model_kwargs,
                tdd_aggregate_fn=tdd_aggregate_fn,
                tdd_energy_fn=tdd_energy_fn,
                tdd_loss_fn=tdd_loss_fn,
                tdd_logsumexp_coef=tdd_logsumexp_coef,
        ...
        if self.int_rew_source in [ModelType.NovelD, ModelType.Count]:
            self.int_rew_model = NovelDModel(
                **int_rew_model_kwargs,
                ...
                policy_cnn=self.features_extractor,
                policy_rnns=self.policy_rnns,
                count_only=self.int_rew_source==ModelType.Count
            )
        if self.int_rew_source == ModelType.E3B:
            self.int_rew_model = E3BModel(
                **int_rew_model_kwargs,
            )
        if self.int_rew_source == ModelType.CTEC:
            print("create the ctec mdoel in the ppo model")
            # pdb.set_trace()
            self.int_rew_model = CTeCModel(
                **int_rew_model_kwargs,
                ctec_energy_fn=ctec_energy_fn,
                ctec_loss_fn=ctec_loss_fn,
                ctec_logsumexp_coef=ctec_logsumexp_coef,
                offpolicy_data=offpolicy_data,
                normalize_repr=self.normalize_repr,

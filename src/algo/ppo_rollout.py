...
from src.utils.enum_types import ModelType, EnvSrc, DecayType
import pdb
...
class PPORollout(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if isinstance(self.observation_space, gym.spaces.Dict):
            if "rgb" in self.observation_space.spaces:
                self.observation_space = self.observation_space["rgb"]
            elif "observation" in self.observation_space.spaces:
    ...
    def on_training_start(self):
        if isinstance(self._last_obs, Dict):
            if "rgb" in self._last_obs:
                self._last_obs = self._last_obs["rgb"]
            elif "observation" in self._last_obs:
        ...
        self.episodic_act_history = [None for _ in range(self.n_envs)]
    ...
    def init_on_rollout_start(self):
        ...
        # update coefficient of intrinsic rewards per each rollout
        decay_rate = DecayType.get_decay_rate(self.int_rew_decay, self._current_progress_remaining)
        self.ppo_rollout_buffer.int_rew_coef = self.int_rew_coef * decay_rate
    ...
    def log_before_transition(self, values):
        ...
        if self.policy.use_status_predictor:
            ...
            for env_id in range(self.n_envs):
                ...
                for pos_id, grid in enumerate(env_grids[env_id].grid):
    ...
    def clear_on_episode_end(self, dones, policy_mems, model_mems):
        for env_id in range(self.n_envs):
            if dones[env_id]:
                ...
                self.episodic_act_history[env_id] = None
    ...
    def log_on_rollout_end(self, log_interval):
        if log_interval is not None and self.iteration % log_interval == 0:
            log_data = {
                ...
                "rollout/number_of_visited_states": self.dd.num_states(),
                "rollout/visited_states_entropy": self.dd.entropy(),
            }
            ...
            if len(self.ep_success_buffer) > 0:
                log_data.update({
                    "rollout/ep_success": safe_mean([int(ep_success) for ep_success in self.ep_success_buffer]),
                })
            else:
                log_data.update({
                    "rollout/ep_success": 0.0,
                })
    ...
    def create_intrinsic_rewards(self, new_obs, actions, dones, **kwargs):
        ...
        with th.no_grad():
            ...
            if "future_obs" in kwargs:
                future_obs_tensor = obs_as_tensor(kwargs["future_obs"], self.device)
        ...
        elif self.int_rew_source == ModelType.TDD:
            if self.policy.int_rew_model.use_ctec_rwd:
                intrinsic_rewards, model_mems = self.policy.int_rew_model.get_ctec_intrinsic_rewards(
                    curr_obs=next_obs_tensor, # use all observtion not just the last one, as we are computing the reward after collecting the data. 
                    future_obs=future_obs_tensor, 
                    next_obs=next_obs_tensor, 
                    last_mems=last_model_mem_tensor,
                    curr_act=curr_act_tensor, 
                    curr_dones=done_tensor, 
        ...
        # NovelD
        elif self.int_rew_source in [ModelType.NovelD, ModelType.Count]:
        ...
        # E3B
        elif self.int_rew_source == ModelType.E3B:
            return self.policy.int_rew_model.get_intrinsic_rewards(
                curr_obs=curr_obs_tensor,
                next_obs=next_obs_tensor,
                last_mems=last_model_mem_tensor,
        ...
        elif self.int_rew_source == ModelType.CTEC:
            if self.policy.int_rew_model.use_etd_rwd:
                intrinsic_rewards, model_mems = self.policy.int_rew_model.get_etd_intrinsic_reward(
                    curr_obs=curr_obs_tensor,
                    next_obs=next_obs_tensor,
                    next_action= self.new_action,
                    last_mems=last_model_mem_tensor,
                    curr_dones=done_tensor,
                    obs_history=self.episodic_obs_emb_history,
                    stats_logger=self.rollout_stats
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                self.new_action, new_values, _, _ = self.policy.forward(new_obs_tensor, policy_mems)
                

            # IR Generation
            if self.int_rew_source == ModelType.TDD:
                if self.policy.int_rew_model.use_ctec_rwd:
                    intrinsic_rewards = rewards # For CTEC-based reward this will be overriden
                    model_mems = None
                else:
        ...
        # print("compute intrinisc reward in the collect reward method")
        # pdb.set_trace()
        ppo_rollout_buffer.compute_future_states()

        if self.int_rew_source == ModelType.TDD:
            if self.policy.int_rew_model.use_ctec_rwd:
                intrinsic_rewards, model_mems = self.create_intrinsic_rewards(ppo_rollout_buffer.observations,ppo_rollout_buffer.actions, dones, future_obs=ppo_rollout_buffer.future_observations)
                ppo_rollout_buffer.intrinsic_rewards = intrinsic_rewards.cpu().detach().numpy()
        ...
        if self.env_source != EnvSrc.Crafter:
            self.dd.update_count(ppo_rollout_buffer.future_observations)
        # import pdb;pdb.set_trace()
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            succ = [x for x in self.ep_success_buffer]
            succ_mean = 0.0 if len(succ) == 0 else np.mean(succ)
            print(f'run: {self.run_id:2d}  '
                  f'iters: {self.iteration}  '
                  f'frames: {self.num_timesteps}  '
                  f'rew: {rew_mean:.6f}  '
                  f'succ: {succ_mean:.6f}  '
                  f'# states: {self.dd.num_states():.6f}  '

...
from src.utils.enum_types import ModelType, DecayType
...
class PPOTrainer(PPORollout):
    ...
    def __init__(
        ...
        int_rew_decay: DecayType = DecayType.NoDecay,
        ...
        if _init_setup_model:
            self._setup_model()
        # print("I am in the last line of ppo traininer init")
    ...
    def train_policy_and_models(self, clip_range, clip_range_vf) -> Tensor:
        ...
        for epoch in range(max(self.n_epochs, self.model_n_epochs)):
            for rollout_data, offpolicy_data in self.ppo_rollout_buffer.get(self.batch_size):
                # Train intrinsic reward models
                if epoch < self.model_n_epochs:
                    if self.policy.int_rew_source == ModelType.TDD:
                ...
                if epoch < self.n_epochs:
                    ...
                    if self.use_sde: # TODO: do not understand

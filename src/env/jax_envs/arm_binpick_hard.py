...
from src.env.jax_envs.arm_envs import ArmEnvs
import os
...
class ArmBinpickHard(ArmEnvs):
    def _get_xml_path(self):
        ...
        return os.path.join(os.path.dirname(os.path.realpath(__file__)), 'assets', "panda_binpick_hard.xml")
    ...
    def _get_obs(self, pipeline_state: base.State, goal: jax.Array, timestep) -> jax.Array:
        ...
        if self.include_goal_in_obs:
            ...
            return jnp.concatenate([q_subset] + [eef_x_pos] + [eef_xd_vel] + [finger_distance] + [gripper_force])

...
class SacTrajectoryUniformSamplingQueue(QueueBase[Sample], Generic[Sample]):
    ...
    def flatten_crl_fn(config, env, transition, sample_key: PRNGKey) :
        if config.use_her:
...
### Trajectories Buffer with CRTR Sampling
class TrajectoryUniformSamplingQueue(QueueBase[Sample], Generic[Sample]):
    """Implements an uniform sampling limited-size replay queue BUT WITH TRAJECTORIES."""

    def __init__(self, *args, repetition_factor: int = 1, **kwargs):
        """
        Args:
            repetition_factor: Number of times to repeat each trajectory for negative sampling.
                             Higher values = more negatives from same trajectory.
        """
        super().__init__(*args, **kwargs)
        self.repetition_factor = repetition_factor

    def sample_internal(self, buffer_state: ReplayBufferState) -> Tuple[ReplayBufferState, Sample, Sample]:
        if buffer_state.data.shape != self._data_shape:
            raise ValueError(
                f"Data shape expected by the replay buffer ({self._data_shape}) does "
                f"not match the shape of the buffer state ({buffer_state.data.shape})"
        ...
        # Calculate base sample size considering repetition factor
        base_shape = self.num_envs
        
        # Sampling envs idxs with repetition for CRTR
        base_envs_idxs = jax.random.choice(
            sample_key, 
            jnp.arange(self.num_envs), 
            shape=(base_shape,), 
            replace=False
        )
        jax.debug.print("env_ids before repeation {x}", x=base_envs_idxs)
        @functools.partial(jax.jit, static_argnames=("rows", "cols"))
        def create_matrix(rows, cols, min_val, max_val, rng_key):
            rng_key, subkey = jax.random.split(rng_key)
            start_values = jax.random.randint(subkey, shape=(rows,), minval=min_val, maxval=max_val)
        ...
        # Sample transitions BEFORE repetition (base_shape trajectories)
        matrix_before = create_matrix(
            base_shape,
            self.episode_length,
            buffer_state.sample_position,
            buffer_state.insert_position - self.episode_length,
        ...
        batch_before = create_batch_vmaped(buffer_state.data[:, base_envs_idxs, :], matrix_before)
        transitions_before = self._unflatten_fn(batch_before)
        
        # Repeat trajectory indices for negative sampling
        envs_idxs = jnp.repeat(base_envs_idxs[:base_envs_idxs.shape[0]//self.repetition_factor], self.repetition_factor, axis=0)
        jax.debug.print("env_ids after repeation {x}", x=envs_idxs)
        shape = envs_idxs.shape[0]

        # Sample transitions AFTER repetition (shape trajectories with repetitions)
        matrix_after = create_matrix(
            shape,
            self.episode_length,
            buffer_state.sample_position,
            buffer_state.insert_position - self.episode_length,
        ...
        batch_after = create_batch_vmaped(buffer_state.data[:, envs_idxs, :], matrix_after)
        transitions_after = self._unflatten_fn(batch_after)
        
        return buffer_state.replace(key=key), transitions_before, transitions_after

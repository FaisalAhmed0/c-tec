...
def crl_task_reward(contrastive_network, contrastive_params, transition: Transition, args, key_critic):
    state = transition.observation[:, :, :args.obs_dim]
    action = transition.action
    if "ant_hardest_maze" in args.env_name:
        future_state = goal = jnp.array([28, 36])[None, :]
    elif "humanoid_u_maze" in args.env_name:
        future_state = goal = jnp.array([6,2,1.25])[None, :]
    elif "arm" in args.env_name: 
        future_state = goal = jnp.array([0.3, 0.6, 0.03])[None, :]
    # future_state = transition.extras["future_state_for_rwd"]

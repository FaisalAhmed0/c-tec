...
def infonce(logits, reweigh_positives=False):
    resubs = True
...
def make_contrastive_critic_loss(crl_networks, args): 
    ...
    def critic_loss(critic_params, transitions, key):
        ...
        if args.use_mono_critic:
            ...
            ################ Contrastive losses ################
            critic_loss = contrastive_losses()[args.contr_loss](logits, args.reweigh_positives)
            ################ Contrastive losses ################
...
from brax.training.types import Params, Policy, Any, Transition, PRNGKey
def make_sac_losses(
...
def make_sac_losses_separate_task_critic(
  ...
  def task_critic_loss(
    ...
    next_obs = transitions.extras["org_next_observation"]
    q_old_action = gc_q_network.apply(normalizer_params, q_params, obs, transitions.action)
    next_dist_params = policy_network.apply(
        normalizer_params, policy_params, next_obs
    )
    next_action = parametric_action_distribution.sample_no_postprocessing(
        next_dist_params, key
    )
    ...
    next_action = parametric_action_distribution.postprocess(next_action)
    next_q = gc_q_network.apply(normalizer_params,target_q_params,next_obs,next_action,)
    next_v = jnp.min(next_q, axis=-1) - alpha * next_log_prob
    target_q = jax.lax.stop_gradient(
  ...
  def actor_loss(
    ...
    intrinsic_q_action = q_network.apply(
        normalizer_params, q_params, obs, action
    )
    # import pdb;pdb.set_trace()
    task_q_action = gc_q_network.apply(
        normalizer_params, task_q_params, obs, action
    )
